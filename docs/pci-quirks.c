...
ex,fdw_dataconv(renesas_fw+data_index-4));
//step 12
fwd_enable(pdev,0);
udelay(100);
//step 13
...
&ports_available);
dev_dbg(&xhci_pdev->dev, "Configurable USB 2.0 ports to hand over to xCHI: 0
x%x\n",
ports_available);
...
if((pdev->vendor == 0x1912)&&(pdev->device == 0x0015)) //Renesas USB3.0 host
controller
{
dev_warn(&pdev->dev, "IEC:Find Renesas USB3.0 host controller\n");
pci_read_config_word(pdev,FW_CONTROL,&reg16b);
if(reg16b & 0x8000)
dev_warn(&pdev->dev, "IEC:Renesas USB3.0 host controller ROM
exists\n");
else
{
dev_warn(&pdev->dev, "IEC:Renesas USB3.0 host controller ROM
don't exists,try to download FW\n");
fdw_renesas(pdev);
}
}
//Wilson modified for usb firmware download [e:2013/9/26]
