...
{		
	...
	if (wdt_disable>1)pm51_wdtset(0); //IEC-ADD by EDEN for wdt timeout reboot system at 20151113
}
                                ...
                                void *p)
{
	...
	if ( event == PM_SUSPEND_PREPARE) {
		...
		pm51_wdtset(0); //IEC-ADD by EDEN for kernel init wdtSetWatchdog 10 Sec at 20151113
		wdt_disable=1;	
	}
	return NOTIFY_DONE;	
}
