	...
	size_t *retlen, u_char *buf)
{
	...
	//printk("\nReading at chip No.%d from 0x%x......\n",flash->spi->chip_select,from);	
	spi_sync(flash->spi, &m);
	...
	size_t *retlen, const u_char *buf)
{
	...
	//printk("Programming chip No.%d at 0x%x\n",flash->spi->chip_select,to);
...
static int __devinit nmyx25_probe(struct spi_device *spi)
{
	...
	flash->mtd.erase = nmyx25_mtd_erase;
	flash->mtd.read = nmyx25_mtd_read;
	flash->mtd.write = nmyx25_mtd_write;
